// ---------------- search.js (FOR NETLIFY FRONTEND) ---------------- //
    ...
    try {
        ...
        buses.forEach(bus => {
            html += `<tr>
                ...
                <td>${bus.amenities && bus.amenities.length ? bus.amenities.join(", ") : "-"}</td>
                <td>${bus.route && bus.route.length ? bus.route.map(stop => `<div>${stop}</div>`).join("") : "-"}</td>
            </tr>`;
        });
        ...
        cheapestDiv.textContent = "-";
        expensiveDiv.textContent = "-";
        mostSeatsDiv.textContent = "-";
    }
}
